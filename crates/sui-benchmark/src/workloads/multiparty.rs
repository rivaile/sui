...
#[derive(Debug)]
pub struct MultipartyTestPayload {
    /// ID of the Move package with multiparty utility functions
    package_id: ObjectID,
...
impl std::fmt::Display for MultipartyTestPayload {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "multiparty")
    }
}

impl Payload for MultipartyTestPayload {
    fn make_new_payload(&mut self, effects: &ExecutionEffects) {
        debug_assert!(
            effects.is_ok(),
            "Multiparty transactions should never abort: {effects:?}",
        );
        ...
        for (obj, owner) in effects.mutated().into_iter().chain(effects.created()) {
            ...
            match owner {
                ...
                _ => unreachable!("multiparty payload never transfers objects to other owners"),
            }
            break;
        }
...
impl MultipartyTestPayload {
    fn create_transaction(&self) -> Transaction {
        ...
        builder.programmable_move_call(
            self.package_id,
            Identifier::new("multiparty").unwrap(),
            // Randomly transfer object to either ConsensusV2 or AddressOwner.
            [
                Identifier::new("transfer_multiparty").unwrap(),
...
#[derive(Debug)]
pub struct MultipartyWorkloadBuilder {
    num_payloads: u64,
}

#[async_trait]
impl WorkloadBuilder<dyn Payload> for MultipartyWorkloadBuilder {
    ...
    async fn build(
        ...
        Box::<dyn Workload<dyn Payload>>::from(Box::new(MultipartyWorkload {
...
impl MultipartyWorkloadBuilder {
    pub fn from(
        workload_weight: f32,
        target_qps: u64,
        num_workers: u64,
...
#[derive(Debug)]
pub struct MultipartyWorkload {
    /// ID of the Move package with multiparty functions
    package_id: ObjectID,
...
#[async_trait]
impl Workload<dyn Payload> for MultipartyWorkload {
    async fn init(
        &mut self,
        ...
        let (first_gas, _) = self
            .init_gas
            .split_first()
            .expect("Not enough gas to initialize multiparty workload");

        let mut path = benchmark_move_base_dir();
        path.push("src/workloads/data/multiparty");
        let SystemState {
            reference_gas_price,
            protocol_config: _,
        ...
        info!("Multiparty package id {:?}", self.package_id);
    }
    ...
    async fn make_test_payloads(
        ...
        for (gas, sender, keypair) in &self.payload_gas[..self.payload_gas.len() / 2] {
            let transaction = TestTransactionBuilder::new(*sender, *gas, reference_gas_price)
                .move_call(self.package_id, "mp", "create_multiparty", vec![])
            ...
            futures.push(async move {
                ...
                let (
                    ...
                    panic!("create_multiparty should always create a ConsensusV2 object");
                };
