...
pub const POOL_RELATED_OBJECTS_PATH: &str = "/opt/sui/pool_related_ids.txt";
...
impl CacheUpdateHandler {
    pub fn new() -> Self {
        println!("Creating CacheUpdateHandler");
    ...
    pub async fn notify_written(&self, objects: Vec<(ObjectID, Object)>) {
        println!("notify_written, {:?}", objects);
...
impl Drop for CacheUpdateHandler {
    fn drop(&mut self) {
        println!("Dropping CacheUpdateHandler");
